...
#define BPP_SUBSAMPLE_DATA

#define NSPECIES        64         /* max # of species */
...
#define NGENE           25000      /* max # of loci */
...
int ReadSeqData(char *seqfile, char *locusratef, char *heredityf, FILE*fout, char cleandata, int *ipop)
{
   ...
   */
...
#if(defined BPP_SUBSAMPLE_DATA)  /***** bpp-subsample-data, 2017.10.8 **/
   int nsub = 3, conf[50000] = { 0 }, ss[10] = { 1, 3, 6 }, h;
   int conf3s = 1;  /* 1 to use 50:25:25 configurations */
   char fsubname[4096] = "subset.txt", fsubIDname[4096] = "IDs.txt", spstr[4096], * p;
   char* confstr[10] = { "123", "113", "223" };
   int sID3s[3][3] = { { 0, 1, 2 }, { 0, 0, 2 }, { 1, 1, 2 } };
   FILE* fsub, * fsubID, *fin;

   printf("#species: species IDs (e.g., 3: 1 5 3)? ");
   scanf("%d:%d%d", &nsub, ss + 0, ss + 1);
   if (nsub > 2) scanf("%d", ss + 2);
   if (nsub > 3) scanf("%d", ss + 3);
   if (nsub > 4) scanf("%d", ss + 4);
   if (nsub != 3) conf3s = 0;
   ss[0]--; ss[1]--;
   if (nsub > 2) ss[2]--;
   if (nsub > 3) ss[3]--;
   if (nsub > 4) ss[4]--;
   if (nsub < 2 || nsub > 5) error2("wrong number of species.");
   for (i = 0; i < nsub; i++)
      if (ss[i] < 0 || ss[i] > sptree.nspecies - 1) error2("wrong species IDs.");

   sprintf(spstr, "%s%s", sptree.nodes[ss[0]].name, sptree.nodes[ss[1]].name);
   for (i = 2; i < nsub; i++)
      strcat(spstr, sptree.nodes[ss[i]].name);
   strcpy(fsubname, com.seqf);
   p = strstr(fsubname, ".txt");
   if (p == NULL) p = fsubname;
   sprintf(p, ".%s.txt", spstr);
   sprintf(fsubIDname, "IDs-%s.txt", spstr);

   fsub = gfopen(fsubname, "w");
   fsubID = gfopen(fsubIDname, "w");
   if (conf3s) {    /* 3s: first half conf 0, second half conf 1 or 2. */
   ...
   for (locus = 0, maxns = 0; locus < data.ngene; ipop += data.ns[locus++]) {
      ...
      printf("locus #%d: %d seqs %4d sites (", locus+1, com.ns, com.ls);
...
#if(defined BPP_SUBSAMPLE_DATA) /***** bpp-subsample-data, 2017.10.8 **/
      char keep[5000] = { 0 };  /* flags to mark sequences in the alignment to be kept. */
      int k, it;
      if (com.ns > 5000) error2("ns > 5000?");
      if (conf3s) {
         fprintf(fsubID, "%d (%s):", locus + 1, confstr[conf[locus]]);
         for (i = 0; i < nsub; i++) {
            /* sample the k-th sequence from ss[i], which is sequence j. */
            int s = ss[sID3s[conf[locus]][i]];  /* sample species s */
            if (data.nseqsp[locus][s] < 1)
               printf("%d seqs from %s at locus %d\n", sptree.nodes[s].name, locus + 1);
            /* sampled the k-th remaining sequence from species s. */
            k = (int)(data.nseqsp[locus][s] * rndu());
            if (i == 1 && (conf[locus] == 1 || conf[locus] == 2)) {  /* conf 113 or 223 */
               if (data.nseqsp[locus][s] < 2)
                  printf("only %d seqs from %s at locus %d\n", sptree.nodes[s].name, locus+1);
               k = (int)((data.nseqsp[locus][s] - 1) * rndu());
            }
            it = 0;
            for (j = 0; j < com.ns; j++) {
               if (ipop[j] != s || keep[j]) continue;
               if (it++ == k) {
                  keep[j] = (char)1;
                  break;
               }
            }
            fprintf(fsubID, "\t%d", j + 1);
         }
      }
      else {
         fprintf(fsubID, "%d:", locus + 1);
         for (i = 0; i < nsub; i++) {
            /* sample the k-th sequence from species s=ss[i], which is sequence j. */
            int s = ss[i];  /* species ID from the species tree */
            if (data.nseqsp[locus][s] < 1)
               printf("%d seqs from %s at locus %d\n", sptree.nodes[s].name, locus + 1);
            k = (int)(data.nseqsp[locus][s] * rndu());
            it = 0;
      ...
      if (k < nsub) printf("%d of %d sequences sampled at locus %d\n", k, nsub, locus + 1);

      fprintf(fsub, "\n\n%2d  %2d", k, com.ls);
      if (conf3s) { /* 3s: 123, 113 or 223 */
         for (i = 0; i < nsub; i++) {
            int s = ss[sID3s[conf[locus]][i]];
            for (j = 0; j < com.ns; j++) {
               if (!keep[j] || ipop[j] != s) continue;
               char* p = strchr(com.spname[j], '^');
               strcpy(p, p + 1);
               sprintf(p + strlen(p), "^%d\0", sID3s[conf[locus]][i] + 1);
               fprintf(fsub, "\n%-30s  ", com.spname[j]);
               for (h = 0; h < com.ls; h++) fprintf(fsub, "%c", com.z[j][h]);
               keep[j] = (char)0;
               break;
            }
         }
...
#if(defined BPP_SUBSAMPLE_DATA)  /***** bpp-subsample-data, 2017.10.8 **/
   printf("\n\nSequence IDs for loci are in %s\n", fsubIDname);
   exit(0);
#endif
...
double CountCoalescentLocus(int locus)
{
   ...
   */
   ...
   double lnp = 0, starttime, endtime, *t=NULL, y, H = (data.est_heredity == 0 ? 1 : data.heredity[locus]);
